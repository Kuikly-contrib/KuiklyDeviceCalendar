...
# 默认 Kuikly Core 版本
DEFAULT_KUIKLY_CORE_VERSION="2.16.0"
...
MODULE_KMP="KuiklyDeviceCalendar"
MODULE_ANDROID="KuiklyDeviceCalendarAndroid"
...
#      hvigorw assembleHar --mode module -p module=kuiklyDeviceCalendarOhos@default -p product=default -p buildMode=release --no-daemon
#      ohpm publish ../KuiklyDeviceCalendarOhos/build/default/outputs/default/kuiklyDeviceCalendarOhos.har
      if [[ -z "${final_versions}" ]]; then
        final_versions="${build_version}"
      else
        final_versions="${final_versions} | ${build_version}"
